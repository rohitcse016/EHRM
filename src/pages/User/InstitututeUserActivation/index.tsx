...
import { requestAddInstituteUser } from '@/pages/InstituteUser/services/api';
...
const CandidateActivation = ({ visible, onClose, selectedRows, isEditable, onSaveSuccess }: any) => {
  ...
  const addCandidate = async (values: any) => {
    try {
      setLoading(true)
      const msg = await requestAddInstituteUser({ ...candidateData, ...values });
      setLoading(false)
        ...
        <Space direction="vertical" style={{ width: '100%' }} size={[0, 48]}>
          <Layout>
            <Header style={headerStyle}>
              <h2>eCSC Institute-User Activation</h2>
            </Header>
